...
| `owt worktree prune` | agent/script가 stale metadata 정리 | `git worktree prune -v`를 실행하고 tab-separated 결과를 출력한다 | Git repo가 아니면 오류 |
...
- Plain CLI stdout은 TUI escape, decorative table, color 없이 tab-separated record로 유지한다.
