...
- CLI prune은 dirty, unmerged, current, bare, detached worktree를 삭제하지 않고 `--dry-run`이 삭제를 수행하지 않는 regression test로 고정한다.
