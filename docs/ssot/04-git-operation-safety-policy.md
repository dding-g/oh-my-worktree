...
| prune | `owt worktree prune` | `git worktree prune -v` + clean/merged worktree scan; `--dry-run`은 `git worktree prune --dry-run -v`와 interactive candidate review | stale metadata를 정리하고 완료된 worktree를 제거하며 모든 worktree 판단 로그를 출력한다. `--dry-run`은 삭제 없이 selected candidate를 기록한다 | non-current, clean, local branch가 `HEAD`에 merge된 worktree만 제거한다. `HEAD` branch worktree와 branch는 삭제하지 않는다 |
...
- CLI prune은 dirty, unmerged, current, HEAD-branch, bare, detached worktree를 삭제하지 않고 `--dry-run`이 삭제를 수행하지 않는 regression test로 고정한다.
